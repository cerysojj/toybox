...
def main(args):
  ...
  for lr, batch_size, opt in product(*hyperparameters.values()):
    model, flatten, hidden_units = create_model("ResNet18", "toybox_pt", 12, device)
    train_loader, val_loader, _, _, _ = get_data_loaders("toybox_pt", args.data_dir, flatten, batch_size)
